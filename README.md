...
Indexing a dataframe using DfEmbedder starts by representing each row in the dataframe as a string that follows the format: `col0_name is col0_value; col1_name is col1_value`.This approach is based on the premise that the stored data is mostly textual with less importance of its structure (reference, for example: [here](https://arxiv.org/pdf/2502.11596) ).The simplicity of this approach is part of what enables it to deal with huge tables (it can easily handle millions of rows). Next, all strings are embedded using a [static embedding model](https://huggingface.co/blog/static-embeddings) (an embedding method that can generate embedding on CPU in blazing speed with very little loss of quality). Finally, it writes data as a table in Lance format.
